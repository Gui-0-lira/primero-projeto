...
if (isset($_POST['cadastrar'])) {
  ...
  if ($usuario==="" || $senha_plana==="" || $ip==="" || $nome_dispositivo==="" || $condominio==="") {
    ...
    $sql = "INSERT INTO dispositivos (usuario, senha, ip, nome_dispositivo, modelo, mac, tipo_dispositivo, condominio, local, observacao)
            VALUES (?,?,?,?,?,?,?,?,?,?)";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("ssssssssss",$usuario,$senha_hash,$ip,$nome_dispositivo,$modelo,$mac,$tipo_dispositivo,$condominio,$local,$observacao);
    if ($stmt->execute()) { $ok=true; $msg="Dispositivo registrado!"; }
...
<meta charset="utf-8"><title>Cadastrar Dispositivo - Multisat</title>
<style>
...
<div class="top-actions">
  ...
  <a class="primary" href="cadastrar_dispositivo.php">+ Adicionar Dispositivo</a>
  <a href="logout.php">Sair</a>
</div>

<div class="container">
  <div class="card">
    <div class="card-header">Cadastrar Dispositivo</div>
    <div class="card-body">
      <?php if($msg): ?>
      ...
      <form method="POST" autocomplete="off">
        <div class="row">
          <div class="field"><label>Usuário (do dispositivo)</label>
            <input type="text" name="usuario" value="<?php echo safe($_POST['usuario'] ?? ''); ?>" required></div>
          <div class="field"><label>Senha (do dispositivo)</label>
            <input type="password" name="senha" required></div>
          <div class="field"><label>IP</label>
            <input type="text" name="ip" value="<?php echo safe($_POST['ip'] ?? ''); ?>" required></div>
          <div class="field"><label>Nome do dispositivo</label>
            <input type="text" name="nome_dispositivo" value="<?php echo safe($_POST['nome_dispositivo'] ?? ''); ?>" required></div>
          <div class="field"><label>Modelo</label>
            <input type="text" name="modelo" value="<?php echo safe($_POST['modelo'] ?? ''); ?>"></div>
          <div class="field"><label>MAC address</label>
            <input type="text" name="mac" value="<?php echo safe($_POST['mac'] ?? ''); ?>"></div>
          <div class="field"><label>Tipo de dispositivo</label>
            <input type="text" name="tipo_dispositivo" value="<?php echo safe($_POST['tipo_dispositivo'] ?? ''); ?>"></div>
          <div class="field"><label>Condomínio</label>
            <input type="text" name="condominio" value="<?php echo safe($_POST['condominio'] ?? ''); ?>" required></div>
        </div>
        ...
        <div class="field">
          ...
          <div class="local-group">
            <?php
              ...
              foreach($opts as $opt){
                $ck = (isset($_POST['local']) && in_array($opt,(array)$_POST['local'])) ? 'checked':'';
        ...
        <div class="field">
          ...
          <textarea name="observacao"><?php echo safe($_POST['observacao'] ?? ''); ?></textarea>
        </div>

        <button class="btn" type="submit" name="cadastrar" value="1">Registrar</button>
      </form>
    </div>
  </div>
</div>
